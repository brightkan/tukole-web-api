...
from api.serializers.metrics import MetricSerializer, UserPerformanceMetricSerializer, GetPointFromMetricSerializer
...
class MetricViewSet(ModelViewSet):
    ...
    @action(methods=['post'], detail=False, url_path='get-points', url_name="get-points",
    ...
    def get_points(self, request):
        ...
        metric = Metric.objects.filter(team=team, action=action, min_time__lte=min_time, max_time__gte=max_time).first()
        if metric:
